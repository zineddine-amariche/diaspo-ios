...
const RenderView = ({ closeModal, projectId }) => {
  const { isSuccess, isError, data, isLoading, message } = useSelector(
    (state) => ({
  ...
  return isLoading ? (
    ...
    >
      <Txt >Loading</Txt>
      <Txt fontSize={13} color={COLORS.silver} style={{marginTop:5}}>Please wait ...</Txt>
    </View>
  ) : (
    <View
      style={{ backgroundColor: COLORS.white, flex: 1, alignItems: "center" }}
    >
      <Space space={20} />
      <Image source={img} />
      <Space space={20} />
